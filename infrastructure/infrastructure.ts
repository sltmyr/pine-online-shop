...
(async () => {
  ...
  await Promise.all([
    async_exec("yarn build", { cwd: "../frontend" }),
    async_exec("zip build.zip payment_api.py", { cwd: "../paymentApi" }),
    async_exec("pip install -r requirements.txt --target='./dependencies/python'", { cwd: "../paymentApi" })
  ])
  await async_exec("zip -r ../dependencies.zip ./*", { cwd: "../paymentApi/dependencies" })
...
class WebAppStack extends cdk.Stack {
  constructor(scope: cdk.App, id: string, props: cdk.StackProps) {
    ...
    const certificateArnUsEast = 'arn:aws:acm:us-east-1:678739632517:certificate/debfbe97-9f0e-4edf-ab6d-dd87da52ad73';
    const certificateArnEuWest = 'arn:aws:acm:eu-west-1:678739632517:certificate/124a607c-16eb-4b6e-9a93-7d4ae69180a7';
    const stripeSecretsArn = 'arn:aws:secretsmanager:eu-west-1:678739632517:secret:stripeKeys-oGhpnr';

    const certificateUsEast = certificatemanager.Certificate.fromCertificateArn(this, 'CertificateUsEast', certificateArnUsEast);
    const certificateEuWest = certificatemanager.Certificate.fromCertificateArn(this, 'CertificateEuWest', certificateArnEuWest);
    const hostedZone = route53.HostedZone.fromLookup(this, 'HostedZone', { domainName });

    new Website(this, 'Website', { domainName, certificate: certificateUsEast, hostedZone })
    new PaymentApi(this, 'PaymentApi', { domainName: paymentApiDomainName, certificate: certificateEuWest, hostedZone, stripeSecretsArn })
  }
};
...
class PaymentApi extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, props: ApiProps) {
    ...
    paymentApiLambda.addToRolePolicy(new iam.PolicyStatement({
    ...
    new route53.ARecord(this, `aliasRecordPaymentApi`, {
      ...
      target: route53.AddressRecordTarget.fromAlias(new targets.ApiGateway(paymentApi)),
      zone: hostedZone
    })
  }
};
